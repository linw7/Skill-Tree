...
#### [Q 11](http://blog.csdn.net/candyliuxj/article/details/6307814) :
    ...
    解答:
        ...
        1. 具体类型，返回该类型所占的空间大小。
        2. 对象，返回对象的实际占用空间大小。
        3. 数组，返回编译时分配的数组空间大小。作为参数时数组退化为指针。
        4. 指针，返回存储该指针所用的空间大小。
...
#### Q 21 :
    ...
    解答:
        ...
        1. 每次都准确加1，结果为最大，200。
        2. 结果为2时步骤：
            (1)a取内存0到寄存器，b取内存0到寄存器；
            (2)a执行99次并写入内存，内存值为99；
            (3)b执行1次并写入内存，内存值被覆盖为1；
            (4)a取内存1到寄存器，b取内存1到寄存器；
            (5)b执行99次并写入内存，内存值为100；
            (6)a执行1次，写入内存，覆盖之前的100，值为2。
        3. 每次计算过程必须是先从内存取数然后计算，之后再重新写入内存。但对各个线程而言，“取数”和“计算”中间可以被另一个线程打断。
